...
use super::{DrawParams, camera::Camera};
    ...
    pub(super) fn draw_bottom_platform(
        ...
        cam.f_half_height / closer_wall_dist * cam.plane_dist;
        let pixels_to_top =
            half_wall_pixel_height * (y_level - ray.origin.y) + cam.y_shearing;
        let draw_from = ((cam.f_half_height + pixels_to_top) as usize)
            .clamp(bottom_draw_bound, top_draw_bound);

        // Draw to:
        let half_wall_pixel_height =
        cam.f_half_height / further_wall_dist * cam.plane_dist;
        ...
        column
            ...
            .for_each(|(y, rgba)| {
                let row_dist = ((ray.origin.y - y_level) / 2.0) * cam.f_height
