...
impl Raycaster {
    pub fn draw_object(&self, hit: RayHit, object: ObjectHit, data: &mut [u8]) {
        ...
        for y in (-self.y_shearing as i32).max(0)..(self.height as i32 - self.y_shearing as i32).min(self.height as i32) {
            let index =
                (self.height as i32 - 1 - y - self.y_shearing as i32)
        ...
        for y in self.height..(self.height as i32-self.y_shearing as i32).max(self.height as i32) as u32 {
            let index =
                (self.height as i32 - 1 - y as i32 - self.y_shearing as i32)
                    .min(self.height as i32 - 1)
                    .max(0) as usize
                    * self.four_width
                    + hit.screen_x as usize * 4;
        ...
        for y in (-self.y_shearing as i32).min(0)..0 {
            let index =
                (self.height as i32 - 1 - y - self.y_shearing as i32)
                    .min(self.height as i32 - 1)
                    .max(0) as usize
