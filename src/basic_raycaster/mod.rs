...
use crate::textures::{TextureArray, TextureDataRef, TextureID};
...
impl<'a> ColumnRenderer<'a> {
    ...
    fn draw(&mut self, column: &mut [u8]) {
        ...
        loop {
            ...
            // The tile ray hit
            let next_tile = self.current_room.segment.get_tile_unchecked(
                self.ray.next_tile.x as usize,
                self.ray.next_tile.z as usize,
            );
    ...
    C: Iterator<Item = &'a mut [u8]>,
{
    ...
    for (column_index, column) in column_iter.enumerate() {
        ...
        let mut column_drawer = ColumnRenderer::new(ray, camera, player, map, textures);

        column_drawer.draw(column);
    }
}

pub fn cast_and_draw2<'a>(
    camera: &Camera,
    player: &Player,
    map: &Map,
    textures: &TextureArray,
    ...
    column_iter.par_chunks_exact_mut(camera.view_height as usize * 3).enumerate().for_each(|(column_index, column)| {
        ...
        loop {
            ...
            let mut draw_platform = |draw_from_dist: f32, draw_to_dist: f32, height: f32, texture_id: TextureID| -> (usize, usize) {
                ...
                let denominator =
                    (height - ray.origin.y) * camera.f_half_height;
            ...
            // Draw ground platform
            let (_, drawn_to) = draw_platform(ray.previous_wall_dist, ray.wall_dist, current_tile.ground_level, current_tile.ground_tex);

            // Draw ceiling platform
            let (drawn_from, _) = draw_platform(ray.wall_dist, ray.previous_wall_dist, current_tile.ceiling_level, current_tile.ceiling_tex);
            ...
            let mut draw_wall = |bottom_level: f32, top_level: f32, texture_id: TextureID| -> (usize, usize) {
                let texture = textures.get_texture_data(texture_id);
                if texture.is_empty() {
                    return (bottom_draw_bound, top_draw_bound);
                }
                ...
                let pixels_to_bottom =
                    half_wall_pixel_height * (ray.origin.y - bottom_level) - camera.y_shearing;
                let pixels_to_top =
            ...
            // Draw bottom wall
            let (_, drawn_to) = draw_wall(next_tile.bottom_level, next_tile.ground_level, next_tile.bottom_wall_tex);

            // Draw top wall
            let (drawn_from, _) = draw_wall(next_tile.ceiling_level, next_tile.top_level, next_tile.top_wall_tex);
            ...
            if let Some(src_dummy_portal) = next_tile.portal {
                ...
                match src_portal.link {
                    ...
                    None => {
                        fill_black(column, bottom_draw_bound, top_draw_bound);
                        break;
                    }
                }
            }
    ...
    // TODO ask ChatGPT how to multithread this.
    /*for (column_index, column) in column_iter.enumerate() {
        let mut ray = Ray::camera_cast(camera, column_index);
        let mut current_room = map.get_room_data(player.current_room_id());
        let mut current_room_dimensions = current_room.segment.dimensions_i64();
